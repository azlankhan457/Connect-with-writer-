...
function LoginForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      router.push(`/verify?purpose=login&email=${encodeURIComponent(email.trim())}&next=${encodeURIComponent(next)}`);
    } catch (err) {
    ...
    >
      ...
      <form noValidate onSubmit={handleSubmit}>
        ...
        <div className="auth-field">
          ...
          <PasswordInput id="login-password" onChange={(e) => setPassword(e.target.value)} value={password} />
        </div>
      ...
      <p className="auth-footer-link">
        Don&apos;t have an account? <Link className="auth-link" href="/signup">Sign up free</Link>
      </p>
    </AuthShell>
  );
}
